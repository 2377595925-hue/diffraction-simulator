...
def compute_intensity_1d(diff_type, diff_mode, lam, a, b, disk_radius, z, screen_half_mm=20):
    ...
    if diff_type == "single_slit":
        ...
        if diff_mode == "fresnel":
            # 菲涅耳单缝衍射: 用Cornu卷线 (两个边缘的菲涅耳积分之差)
            half_a = a_m / 2
            u1 = (x_line * 1e-3 - half_a) * np.sqrt(2 / (lam * z))
            u2 = (x_line * 1e-3 + half_a) * np.sqrt(2 / (lam * z))
            S1, C1 = fresnel(u1)
            S2, C2 = fresnel(u2)
        ...
        if diff_mode == "fresnel":
            # 菲涅耳圆屏衍射: 泊松亮斑 (用Cornu卷线)
            # 屏上各点的强度取决于该点到圆屏边缘的距离
            rho = np.abs(x_line) * 1e-3 * np.sqrt(2 / (lam * z))
            C_p, S_p = fresnel(rho)
            I = 0.5 * ((0.5 - C_p)**2 + (0.5 - S_p)**2)
        else:
            # Fraunhofer 圆屏衍射 — 巴比涅原理: I_disk = |1 - U_hole|²
            # 注意: 用复振幅（含相位），不是强度
            D = 2 * a_m
            theta = np.arctan(np.abs(x_line) * 1e-3 / z)
            u = np.pi * D * np.sin(theta) / lam
            u = np.where(np.abs(u) < 1e-10, 1e-10, u)
            U_hole = 2 * j1(u) / u  # 圆孔复振幅 (忽略常数相位)
            I = np.abs(1.0 - U_hole) ** 2
...
def compute_pattern_2d(diff_type, diff_mode, lam, a, b, disk_radius, z, 
    ...
    if diff_type == "single_slit":
        if diff_mode == "fresnel":
            half_a = a * 1e-3 / 2
            u1 = (X_s * 1e-3 - half_a) * np.sqrt(2 / (lam * z))
            u2 = (X_s * 1e-3 + half_a) * np.sqrt(2 / (lam * z))
            S1, C1 = fresnel(u1)
            S2, C2 = fresnel(u2)
            I = 0.5 * ((C2 - C1)**2 + (S2 - S1)**2)
        else:
    ...
    # 提升对比度
    I_disp = np.clip(I, 0, 1) ** 0.35
    brightness = np.clip(I_disp * 1.8, 0, 1)
    
    rgb = np.zeros((n_pixels, n_pixels, 3), dtype=np.float32)
...
def calculate():
    ...
    if diff_type == "single_slit":
        if diff_mode == "fresnel":
            screen_half = max(30, z * 1e3 * 5)
        else:
            first_min_mm = z_eff * 1e3 * lam / (aperture * 1e-3)
            screen_half = max(first_min_mm * 8, 8)
    elif diff_type == "circular" or diff_type == "circular_disk":
        D = (aperture if diff_type == "circular" else disk_radius * 2) * 1e-3
        if diff_mode == "fresnel" and diff_type == "circular_disk":
            screen_half = max(15, z * 1e3 * np.sqrt(lam / D) * 2) if D > 0 else 15
        elif diff_mode == "fresnel" and diff_type == "circular":
            screen_half = max(20, z * 1e3 * 4) if D > 0 else 20
        else:
